...
def main():
    ...
    # Cost robustness cases for LQG only, normalized by beta=0 Gaussian baseline.
    ...
    plt.title("LQG cost ratio vs beta=0 baseline")
    plt.ylabel("J(case) / J(beta=0)")
    plt.savefig(f"{figs_dir}/cost.png")

    # State robustness cases for LQG only, normalized by beta=0 Gaussian baseline.
    ...
    plt.xticks([1, 2, 3], ["beta_0.5", "beta_1.0", "diff_noise"])
    plt.title("LQG terminal-state norm ratio vs beta=0 baseline")
