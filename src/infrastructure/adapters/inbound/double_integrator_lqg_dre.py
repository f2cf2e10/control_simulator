...
def main():
    ...
    # Cost robustness cases for LQG-DRE only, normalized by beta=0 Gaussian baseline.
    ...
    plt.title("LQG-DRE cost ratio vs beta=0 baseline")
    plt.ylabel("J(case) / J(beta=0)")
    plt.savefig(f"{figs_dir}/cost.png")

    # State robustness cases for LQG-DRE only, normalized by beta=0 Gaussian baseline.
    ...
    plt.xticks([1, 2, 3], ["beta_0.5", "beta_1.0", "diff_noise"])
    plt.title("LQG-DRE terminal-state norm ratio vs beta=0 baseline")
